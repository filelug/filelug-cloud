 ...
 * @version 1.0
 */
@WebServlet(name = "disconnect", displayName = "disconnect", description = "Disconnect request between repository and server", urlPatterns = {"/disconnect"})
public class DisconnectServlet extends HttpServlet {

    private static final Logger LOGGER = (Logger) LoggerFactory.getLogger("DISCONNECT");

    private static final long serialVersionUID = 7617618159395793969L;
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        try {
            ...
            if (userId == null || encryptedPassword == null || nickname == null || verification == null || computerId == null
                || userId.trim().length() < 1 || encryptedPassword.trim().length() < 1 || nickname.trim().length() < 1 || verification.trim().length() < 1 || computerId < 0) {
                ...
                if (computer == null) {
                    ...
                    if (user == null) {
                        ...
                        if (verified == null || !verified) {
                            ...
                            if (encryptedPassword.equals(foundPasswd)) {
                                ...
                                if (!verification.equals(Utility.generateVerification(userId, encryptedPassword, nickname))) {
                                    LOGGER.warn("User: " + userId + " is testing verification code for disconnect");

                                    String errorMessage = ClopuccinoMessages.getMessage("error");

                                    resp.setContentType(Constants.CONTENT_TYPE_TEXT_PLAIN_UTF8);
                                    resp.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
                                    resp.getWriter().write(String.valueOf(errorMessage));
                                    resp.getWriter().flush();
                                } else if (!userId.equals(computer.getUserId())) { // make sure the admin of the computer is the correct one
                                    String errorMessage = ClopuccinoMessages.localizedMessage(localeString, "user.not.computer.admin", computer.getComputerName(), user.getNickname());
                                ...
                                String errorMessage = ClopuccinoMessages.localizedMessage(localeString, "password.not.correct", user.getPhoneNumber());
