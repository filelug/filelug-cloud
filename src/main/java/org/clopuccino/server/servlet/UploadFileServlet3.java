 ...
 * @version 1.0
 */
@WebServlet(name = "upload-file-from-desktop3", displayName = "upload-file-from-desktop3", description = "Upload file from desktop (V3)", urlPatterns = {"/directory/supload3"})
public class UploadFileServlet3 extends HttpServlet {

    private static final Logger LOGGER = (Logger) LoggerFactory.getLogger(String.valueOf(Sid.DOWNLOAD_FILE2_V2));

    private static final long serialVersionUID = 7150634065451059693L;
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        // check authorizaiton header values

        String downloadKey = req.getHeader(Constants.HTTP_HEADER_NAME_UPLOAD_KEY);

        if (downloadKey == null || ClientDownloadResponseUtility.get(downloadKey) == null) {
            resp.setContentType(Constants.CONTENT_TYPE_TEXT_PLAIN_UTF8);
            resp.setStatus(HttpServletResponse.SC_BAD_REQUEST);
            resp.getWriter().write("file transfer key not provided.");
