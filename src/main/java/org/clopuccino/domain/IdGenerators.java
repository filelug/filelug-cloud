...
public class IdGenerators {
     ...
     * Seed the random number
     */
    public IdGenerators(String generatorSeed) {
        // Start with the current system time as a seed
        long seed = System.currentTimeMillis();

        if (generatorSeed != null && generatorSeed.trim().length() > 0) {
            seed = seed + generatorSeed.hashCode();
        }
...
//    /**
//     * Generate and return a new session identifier using default session id length.
//     * @return A new page id string
//     */
//    public synchronized String generateId() {
//        Integer length = DomainConstants.DEFAULT_CLIENT_SESSION_ID_LENGTH;
//
//        byte[] buffer = new byte[length];
//
//        // Render the result as a String of hexadecimal digits
